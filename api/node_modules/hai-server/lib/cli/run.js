...
function createApp(db, routes, middlewares, auth, argv) {
  ...
  let router = haijsServer.router(db, foreignKeySuffix ? {
    foreignKeySuffix
  } : undefined, auth);
  const defaultsOpts = {
    logger: !argv.quiet,
  ...
  if (auth) {
    ...
    let authenticatedUsers = auth.users || []; // Use this for access token
  ...
  function start(cb) {
    ...
    return load(source).then(db => {
      ...
      process.on('uncaughtException', error => {
        if (error.errno === 'EADDRINUSE') console.log(chalk.red(`Cannot bind to the port ${error.port}. Please specify another port number either through --port argument or through the hai-server.json configuration file`));else console.log('Some error occurred', error);
        process.exit(1);
      });
    });
  } // Start server
